const accounts = [
  ...
  {email:'sgs@audit.tw', name:'SGS 稽核承辦', role:'稽核方 · 尚未授權', tone:'稽核'},
];
const workflow = [
...
const state={token:localStorage.getItem('claim_token')||'',user:null,caseId:'',case:null,status:null};
...
async function login(email){
  try{const data=await api('/login',{method:'POST',body:JSON.stringify({email,password:'demo1234'})});state.token=data.token;state.user=data.user;localStorage.setItem('claim_token',state.token);await enterApp();}
...
async function enterApp(){
  ...
  }catch(err){logout();toast(`工作階段失效：${err.message}`)}
}
async function loadCase(){
  try{state.case=await api(`/cases/${state.caseId}`);$('#case-kicker').textContent=`案件 ${state.case.case_id}`;$('#case-title').textContent=state.case.title;renderPrincipal();renderWorkflow();await loadAudit();}
...
function renderWorkflow(){
  ...
  $$('.workflow-card').forEach((b,i)=>b.onclick=()=>act(b.dataset.tool,workflow[i][4]));
  $('#submit-claim').disabled=!state.case.grant;
}
async function act(tool,args={},confirmed=false){
  try{const r=await api(`/cases/${state.caseId}/act`,{method:'POST',body:JSON.stringify({tool,args,confirmed})});showResult(r);await loadAudit();if(r.code==='NEEDS_HUMAN_CONFIRMATION')$('#confirm-dialog').showModal();return r;}
...
function logout(){state.token='';state.user=null;localStorage.removeItem('claim_token');$('#app-view').classList.add('hidden');$('#login-view').classList.remove('hidden')}
function escapeHtml(v){return String(v).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]))}

renderAccounts();$$('.nav-item').forEach(n=>n.onclick=()=>switchTab(n.dataset.tab));
$('#logout').onclick=logout;$('#case-select').onchange=async e=>{state.caseId=e.target.value;await loadCase()};
